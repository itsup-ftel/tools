...
powershell -Command "Invoke-WebRequest -Uri 'https://aka.ms/SaRA_CommandLineVersion' -OutFile '$env:TEMP\SaRA.zip'; Expand-Archive -Path '$env:TEMP\SaRA.zip' -DestinationPath '$env:TEMP\SaRA' -Force; Start-Process -FilePath '$env:TEMP\SaRA\GetHelpCmd.exe' -ArgumentList '-S ResetOfficeActivation -AcceptEula -CloseOffice' -Wait -NoNewWindow" >nul 2>&1
