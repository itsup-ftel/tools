...
echo      %G%5.%Res% Kiem tra Bitlocker   %G%11.%Res% Restart Explorer    %G%17.%Res% Xem Pass Wi-Fi       %G%23.%Res% ----license-------
...
powershell -Command "$w = cscript //nologo $env:windir\system32\slmgr.vbs /dli; $x = Get-CimInstance -Namespace root\Cimv2\Applications\WindowsLicensing -ClassName SoftwareLicensingProduct -Filter \"ApplicationID='55c92734-d682-4d71-983e-d6ec3f16059f' and PartialProductKey is not null\" -ErrorAction SilentlyContinue; if (-not $x) { Write-Host '  [-] Ket qua: WINDOWS CHUA KICH HOAT (He thong khong co Key)' -ForegroundColor Yellow } else { $Status = $x.LicenseStatus; $Channel = $x.ProductKeyChannel; $isKMS = ($w -match 'KMS' -or $x.Description -match 'VOLUME_KMS'); $isVolume = ($Channel -match 'Volume'); $isOEM = ($Channel -match 'OEM'); $OEMObj = Get-CimInstance -ClassName Win32_ComputerSystemProduct -ErrorAction SilentlyContinue; $isFakeOEM = ($isOEM -and (-not $OEMObj.DigitalProductId)); if ($Status -ne 1) { Write-Host '  [-] Ket qua: WINDOWS CHUA KICH HOAT HOAC HET HAN DUNG THU' -ForegroundColor Yellow } elif ($isKMS) { Write-Host '  [-] Ket qua: WINDOWS CRACK (Su dung may chu KMS gia lap hoac tool AIO)' -ForegroundColor Red } elif ($isVolume) { Write-Host '  [-] Ket qua: WINDOWS CRACK (Su dung tool MAS nap Key gia lap ky thuat so HWID)' -ForegroundColor Red } elif ($isFakeOEM) { Write-Host '  [-] Ket qua: WINDOWS CRACK (Gia lap Key OEM, bo mach chu khong co chip ban quyen goc)' -ForegroundColor Red } else { Write-Host '  [+] Ket qua: BAN QUYEN XIN TUYET DOI (Giay phep Retail hoac OEM chinh hang)' -ForegroundColor Green } }"

echo:
echo %C% KIEM TRA BAN QUYEN MICROSOFT OFFICE:%Res%
powershell -Command "$paths = @(); Get-ChildItem -Path 'C:\Program Files\Microsoft Office\Office*', 'C:\Program Files (x86)\Microsoft Office\Office*', 'C:\Program Files\Microsoft Office\root\Office*', 'C:\Program Files (x86)\Microsoft Office\root\Office*' -ErrorAction SilentlyContinue | ForEach-Object { if (Test-Path \"$($_.FullName)\ospp.vbs\") { $paths += \"$($_.FullName)\ospp.vbs\" } }; if ($paths.Count -eq 0) { Write-Host '  [-] Ket qua: KHONG TIM THAY PHEN BAN OFFICE NAO TREN MAY' -ForegroundColor Yellow } else { $ohook = Test-Path '$env:SystemRoot\system32\spp\tokens\skus\Office*'; if ($ohook) { Write-Host '  [-] Ket qua: OFFICE CRACK (Su dung engine OHOOK gia lap chung chi Office 365/2021)' -ForegroundColor Red } else { $activePath = $paths[0]; $o = cscript //nologo \"$activePath\" /dstatus; if ($o -match 'LICENSE STATUS: LICENSED') { if ($o -match 'VOLUME_KMS' -or $o -match 'KMS host') { Write-Host '  [-] Ket qua: OFFICE CRACK (Su dung may chu KMS gia lap hoac tool AIO Tools)' -ForegroundColor Red } elseif ($o -match 'RETAIL') { Write-Host '  [+] Ket qua: BAN QUYEN XIN THUONG MAI (Giay phep Retail nhap Product Key)' -ForegroundColor Green } else { Write-Host '  [+] Ket qua: BAN QUYEN DOANH NGHIEP (Su dung khoa Commercial Volume khoi doanh nghiep)' -ForegroundColor Green } } else { $vScript = \"$env:ProgramFiles\Microsoft Office\Office16\vnextdiag.ps1\"; if (-not (Test-Path $vScript)) { $vScript = \"$env:ProgramFiles(x86)\Microsoft Office\Office16\vnextdiag.ps1\" }; if (Test-Path $vScript) { $vOut = powershell -ExecutionPolicy Bypass -File \"$vScript\" -List 2>&1; if ($vOut -match 'OlsLicenseState_Licensed') { Write-Host '  [+] Ket qua: BAN QUYEN XIN (Xac thuc ky thuat so qua Tai khoan Microsoft 365 Cloud)' -ForegroundColor Green } else { Write-Host '  [-] Ket qua: OFFICE CHUA KICH HOAT HOAC HET HAN BAN QUYEN' -ForegroundColor Yellow } } else { Write-Host '  [-] Ket qua: OFFICE CHUA KICH HOAT HOAC HET HAN BAN QUYEN' -ForegroundColor Yellow } } } }"


echo:
echo %C% KIEM TRA GOI PHAN MEM ADOBE:%Res%
powershell -Command "$aDir = 'C:\\Program Files\\Adobe'; if (-not (Test-Path $aDir)) { Write-Host '  [-] Ket qua: KHONG CO PHAN MEM ADOBE NAO TREN MAY' -ForegroundColor Gray } else { $hosts = Get-Content $env:windir\system32\drivers\etc\hosts -ErrorAction SilentlyContinue; $h = $hosts -match 'adobe.com' -or $hosts -match 'adobe.io'; $exe = Get-ChildItem -Path $aDir -Filter '*.exe' -Recurse -ErrorAction SilentlyContinue | Where-Object { $_.DirectoryName -notmatch 'Acrobat' } | Select-Object -First 5; if (-not $exe) { Write-Host '  [+] Ket qua: Thu muc Adobe trong hoac chi co Adobe Reader free' -ForegroundColor Green } else { $crack = $false; foreach ($e in $exe) { $sig = Get-AuthenticodeSignature $e.FullName -ErrorAction SilentlyContinue; if ($sig.Status -and $sig.Status -ne 'Valid') { $crack = $true } }; if ($crack) { Write-Host '  [-] Ket qua: ADOBE CRACK (Phat hien ma nhi phan loi / Vo hieu hoa chu ky so goc boi tool GenP hoac Monkrus)' -ForegroundColor Red } else { if ($h) { Write-Host '  [-] Ket qua: ADOBE CRACK (File Hosts bi sua doi co tinh de chan may chu kiem tra ban quyen)' -ForegroundColor Red } else { Write-Host '  [+] Ket qua: BAN QUYEN XIN 100% (File he thong con nguyen ven chu ky so cua Adobe Inc / Dang dung Creative Cloud)' -ForegroundColor Green } } } }"

echo:
echo %C% KIEM TRA TOAN BO PHAN MEM AUTODESK:%Res%
powershell -Command "$aDir='C:\\Program Files\\Autodesk'; if(-not (Test-Path $aDir)){Write-Host '  [-] Ket qua: KHONG CO PHAN MEM AUTODESK NAO TREN MAY' -ForegroundColor Gray}else{$lmgrd=Get-Process -Name 'lmgrd','adskflex' -ErrorAction SilentlyContinue; $clic=Test-Path 'C:\\Program Files (x86)\\Common Files\\Autodesk Shared\\AdskLicensing\\adskflex_*.lic'; $hosts=Get-Content $env:windir\system32\drivers\etc\hosts -ErrorAction SilentlyContinue; $isHostsBlock=$hosts -match 'autodesk.com'; $svc=Get-Service -Name 'AdskLicensingService' -ErrorAction SilentlyContinue; $isSvcStop=($svc -and ($svc.Status -ne 'Running')); $isCrackNLM=($lmgrd -or $clic); $isInvalidSig=$false; $exe=Get-ChildItem -Path $aDir -Filter 'acad.exe' -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1; if($exe){$sig=Get-AuthenticodeSignature $exe.FullName -ErrorAction SilentlyContinue; if($sig.Status -and ($sig.Status -ne 'Valid')){$isInvalidSig=$true}}; if($isCrackNLM){Write-Host '  [-] Ket qua: AUTODESK CRACK (Su dung Network License Manager gia lap offline boi tool X-Force)' -ForegroundColor Red}elif($isHostsBlock){Write-Host '  [-] Ket qua: AUTODESK CRACK (Chan IP nhan dien ban quyen lau cua hang tren file Hosts)' -ForegroundColor Red}elif($isSvcStop){Write-Host '  [-] Ket qua: AUTODESK CRACK (Dich vu cap phep AdskLicensingService bi tat cuong buc de bypass)' -ForegroundColor Red}elif($isInvalidSig){Write-Host '  [-] Ket qua: AUTODESK CRACK (File thuc thi acad.exe bi ghi de mat chu ky so goc)' -ForegroundColor Red}else{Write-Host '  [+] Ket qua: BAN QUYEN XIN 100% (He thong sach, dich vu goc chay on dinh, su dung License hop phap)' -ForegroundColor Green}}"

echo:
echo %C% KIEM TRA CAC UNG DUNG PDF CHUYEN DUNG:%Res%
:: --- Foxit PDF Editor & PhantomPDF (Chinh xac tuyet doi) ---
powershell -Command "$fDirs = Get-ChildItem -Path 'C:\Program Files (x86)\Foxit Software\*', 'C:\Program Files\Foxit Software\*' -ErrorAction SilentlyContinue | Where-Object { $_.PSIsContainer -and ($_.Name -match 'Editor' -or $_.Name -match 'PhantomPDF') }; foreach ($d in $fDirs) { $exe = Get-ChildItem -Path $d.FullName -Filter 'Foxit*.exe' -ErrorAction SilentlyContinue | Select-Object -First 1; if ($exe) { $crack = $false; $files = Get-ChildItem -Path $d.FullName -Filter *.dll -ErrorAction SilentlyContinue | ForEach-Object { $_.FullName }; $files += $exe.FullName; foreach ($f in $files) { $sig = Get-AuthenticodeSignature $f -ErrorAction SilentlyContinue; if ($sig.Status -and $sig.Status -ne 'Valid' -and $sig.SignerCertificate.Subject -notmatch 'Foxit') { $crack = $true } }; if ($crack) { Write-Host \"  [-] Ket qua: $($d.Name) BI CRACK (Phat hien Module can thiep he thong hoac file mat chu ky so gốc)\" -ForegroundColor Red } else { Write-Host \"  [+] Ket qua: $($d.Name) BAN QUYEN SACH (Su dung file goc va chu ky so hop le)\" -ForegroundColor Green } } }"

:: --- Nitro PDF Pro (Chinh xac tuyet doi) ---
powershell -Command "$nDirs = Get-ChildItem -Path 'C:\Program Files\Nitro\*', 'C:\Program Files (x86)\Nitro\*' -ErrorAction SilentlyContinue | Where-Object { $_.PSIsContainer -and ($_.Name -match 'Pro') }; foreach ($d in $nDirs) { $exe = Get-ChildItem -Path $d.FullName -Filter 'NitroPDF.exe' -ErrorAction SilentlyContinue | Select-Object -First 1; if ($exe) { $crack = $false; $files = Get-ChildItem -Path $d.FullName -Filter *.dll -ErrorAction SilentlyContinue | ForEach-Object { $_.FullName }; $files += $exe.FullName; foreach ($f in $files) { $sig = Get-AuthenticodeSignature $f -ErrorAction SilentlyContinue; if ($sig.Status -and $sig.Status -ne 'Valid' -and $sig.SignerCertificate.Subject -notmatch 'Nitro') { $crack = $true } }; if ($crack) { Write-Host \"  [-] Ket qua: Nitro PDF ($($d.Name)) BI CRACK (File thuc thi bi va ma nguon bơi tool crack)\" -ForegroundColor Red } else { Write-Host \"  [+] Ket qua: Nitro PDF ($($d.Name)) BAN QUYEN SACH (Ung dung dung file goc chu ky tron vẹn)\" -ForegroundColor Green } } }"

:: --- ABBYY FineReader (Chinh xac tuyet doi) ---
powershell -Command "$aDirs = Get-ChildItem -Path 'C:\Program Files\ABBYY FineReader *', 'C:\Program Files (x86)\ABBYY FineReader *' -ErrorAction SilentlyContinue | Where-Object { $_.PSIsContainer }; foreach ($d in $aDirs) { $exe = Get-ChildItem -Path $d.FullName -Filter 'FineReader.exe' -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1; if ($exe) { $crack = $false; $aSvc = Get-Service -Name 'ABBYY FineReader * Licensing Service' -ErrorAction SilentlyContinue | Where-Object { $_.DisplayName -match $d.Name.Split(' ')[-1] } | Select-Object -First 1; if ($aSvc -and $aSvc.Status -ne 'Running') { $crack = $true } else { $checkList = @($exe.FullName); $commonLic = 'C:\Program Files\Common Files\ABBYY\FineReader'; if (Test-Path $commonLic) { $checkList += (Get-ChildItem -Path $commonLic -Include *.dll,*.exe -Recurse -ErrorAction SilentlyContinue | ForEach-Object { $_.FullName }) }; foreach ($f in $checkList) { $sig = Get-AuthenticodeSignature $f -ErrorAction SilentlyContinue; if ($sig.Status -and $sig.Status -ne 'Valid' -and $sig.SignerCertificate.Subject -notmatch 'ABBYY') { $crack = $true } } }; if ($crack) { Write-Host \"  [-] Ket qua: $($d.Name) BI CRACK (Dich vu cap phep bi dong bang hoac nho tệp tin gia lap)\" -ForegroundColor Red } else { Write-Host \"  [+] Ket qua: $($d.Name) BAN QUYEN SACH (Module ma hoa dung chuan file cua hang)\" -ForegroundColor Green } } }"
echo:
echo %C%====================================================================%Res%
