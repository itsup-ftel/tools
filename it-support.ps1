...
powershell -Command "$paths = @(); $reg = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\OfficeSoftwareProtectionPlatform' -Name 'InstallRootAndProductVersion' -ErrorAction SilentlyContinue; if($reg){ $paths += (Join-Path $reg.InstallRootAndProductVersion 'ospp.vbs') }; $paths += @('${env:SystemDrive}\Program Files\Microsoft Office\Office*', '${env:SystemDrive}\Program Files (x86)\Microsoft Office\Office*') | Resolve-Path -ErrorAction SilentlyContinue | Loop-Through { Join-Path $_.Path 'ospp.vbs' }; $vbs = $paths | Where-Object { Test-Path $_ } | Select-Object -Unique; if($vbs){ $found=$false; foreach($p in $vbs){ $res = cscript //nologo \"$p\" /dstatus; $keys = $res | Select-String 'Last 5 characters'; if($keys){ $found=$true; foreach($k in $keys){ Write-Host \"[+] Key found at [$(Split-Path $p -Parent)]: $($k.Line.Trim())\" -ForegroundColor Green } } }; if(-not $found){ Write-Host '[-] No Office keys detected via OSPP.' -ForegroundColor Yellow } } else { Write-Host '[-] Office Licensing Tool (ospp.vbs) not found on this system.' -ForegroundColor Red }"
