...
echo      %G%5.%Res% Kiem tra Bitlocker   %G%11.%Res% Restart Explorer    %G%17.%Res% Xem Pass Wi-Fi       %G%23.%Res% ----14th5-------
...
powershell -Command "$w=cscript //nologo $env:windir\system32\slmgr.vbs /dli; $x=Get-CimInstance -Namespace root\Cimv2\Applications\WindowsLicensing -ClassName SoftwareLicensingProduct -Filter \"ApplicationID='55c92734-d682-4d71-983e-d6ec3f16059f' and PartialProductKey is not null\" -ErrorAction SilentlyContinue; if(!$x){Write-Host '  [-] Ket qua: WINDOWS CHUA KICH HOAT (He thong trong troi, khong co Key)' -ForegroundColor Yellow}else{$KMS=$w -match 'KMS' -or $x.Description -match 'VOLUME_KMS'; $Channel=$x.ProductKeyChannel; $Status=$x.LicenseStatus; if($Status -ne 1){Write-Host '  [-] Ket qua: WINDOWS CHUA KICH HOAT HOAC HET HAN DUNG THU' -ForegroundColor Yellow}elseif($KMS){Write-Host '  [-] Ket qua: WINDOWS CRACK (Su dung may chu KMS gia lap hoac tool AIO)' -ForegroundColor Red}elseif($Channel -match 'Retail' -or $Channel -match 'OEM'){$OEM=Get-CimInstance -ClassName Win32_ComputerSystemProduct -ErrorAction SilentlyContinue; if($Channel -match 'OEM' -and !$OEM.DigitalProductId){Write-Host '  [-] Ket qua: WINDOWS CRACK (Gia lap Key OEM, may khong co chip bản quyen goc)' -ForegroundColor Red}else{Write-Host '  [+] Ket qua: BAN QUYEN XIN 100% (Giay phep Retail / OEM hop phap tu nha san xuat)' -ForegroundColor Green}}elseif($Channel -match 'Volume'){Write-Host '  [-] Ket qua: WINDOWS CRACK (Su dung tool MAS nap Key gia lap ky thuat so HWID)' -ForegroundColor Red}else{Write-Host '  [-] Ket qua: WINDOWS CRACK (Khong trung khop kenh phan phoi hop le)' -ForegroundColor Red}}"

echo:
echo %C% KIEM TRA BAN QUYEN MICROSOFT OFFICE:%Res%
powershell -Command "$ohook=Test-Path '$env:SystemRoot\system32\spp\tokens\skus\Office*'; $paths=@('${env:ProgramFiles}\Microsoft Office\Office16','${env:ProgramFiles(x86)}\Microsoft Office\Office16'); $status='NOT_FOUND'; $channel='NONE'; foreach($p in $paths){if(Test-Path \"$p\ospp.vbs\"){$o=cscript //nologo \"$p\ospp.vbs\" /dstatus; if($o -match 'LICENSE STATUS: LICENSED'){$status='LICENSED'; if($o -match 'VOLUME_KMS' -or $o -match 'KMS host'){$channel='KMS'}elseif($o -match 'RETAIL'){$channel='RETAIL'}else{$channel='VOLUME'}}}}; if($ohook){Write-Host '  [-] Ket qua: OFFICE CRACK (Su dung engine OHOOK gia lap chung chi Office 365/2021)' -ForegroundColor Red}elif($status -eq 'NOT_FOUND'){Write-Host '  [-] Ket qua: KHONG TIM THAY PHEN BAN OFFICE NAO TREN MAY' -ForegroundColor Yellow}elif($status -ne 'LICENSED'){Write-Host '  [-] Ket qua: OFFICE CHUA KICH HOAT HOAC HET HAN BAN QUYEN' -ForegroundColor Yellow}elif($channel -eq 'KMS'){Write-Host '  [-] Ket qua: OFFICE CRACK (Su dung may chu KMS gia lap hoac tool AIO Tools)' -ForegroundColor Red}elif($channel -eq 'RETAIL'){Write-Host '  [+] Ket qua: BAN QUYEN XIN 100% (Giay phep Retail hop phap hoac Tai khoan Microsoft 365)' -ForegroundColor Green}elif($channel -eq 'VOLUME'){Write-Host '  [+] Ket qua: BAN QUYEN DOANH NGHIEP (Su dung khoa Commercial Volume hop le)' -ForegroundColor Green}else{Write-Host '  [-] Ket qua: OFFICE CRACK (Vi pham kenh xac thuc ban quyen)' -ForegroundColor Red}"

echo:
echo %C% KIEM TRA GOI PHAN MEM ADOBE:%Res%
powershell -Command "$aDir='C:\Program Files\Adobe'; if(!(Test-Path $aDir)){Write-Host '  [-] Ket qua: KHONG CO PHAN MEM ADOBE NAO TREN MAY' -ForegroundColor Gray}else{$hosts=Get-Content $env:windir\system32\drivers\etc\hosts -ErrorAction SilentlyContinue; $h=$hosts -match 'adobe.com' -or $hosts -match 'adobe.io'; $exe=Get-ChildItem -Path $aDir -Filter '*.exe' -Recurse -ErrorAction SilentlyContinue | Where-Object {$_.DirectoryName -notmatch 'Acrobat'} | Select-Object -First 5; if(!$exe){Write-Host '  [+] Ket qua: Thu muc Adobe trống hoac chi co Adobe Reader free' -ForegroundColor Green}else{$crack=$false; foreach($e in $exe){$sig=Get-AuthenticodeSignature $e.FullName -ErrorAction SilentlyContinue; if($sig.Status -and $sig.Status -ne 'Valid'){$crack=$true}}; if($crack){Write-Host '  [-] Ket qua: ADOBE CRACK (Phat hien ma nhi phan loi / Vo hieu hoa chu ky so goc bơi tool GenP hoac Monkrus)' -ForegroundColor Red}elseif($h){Write-Host '  [-] Ket qua: ADOBE CRACK (File Hosts bi sua doi co tinh de chan may chu kiem tra bản quyen)' -ForegroundColor Red}else{Write-Host '  [+] Ket qua: BAN QUYEN XIN 100% (File he thong con nguyen vẹn chu ky so cua Adobe Inc / Dang dung Creative Cloud)' -ForegroundColor Green}}}"

echo:
echo %C% KIEM TRA GOI PHAN MEM AUTODESK:%Res%
powershell -Command "$aDir='C:\Program Files\Autodesk'; if(!(Test-Path $aDir)){Write-Host '  [-] Ket qua: KHONG CO PHAN MEM AUTODESK/AUTOCAD NAO TREN MAY' -ForegroundColor Gray}else{$lmgrd=Get-Process -Name 'lmgrd','adskflex' -ErrorAction SilentlyContinue; $clic=Test-Path 'C:\Program Files (x86)\Common Files\Autodesk Shared\AdskLicensing\adskflex_*.lic'; $svc=Get-Service -Name 'AdskLicensingService' -ErrorAction SilentlyContinue; $hosts=Get-Content $env:windir\system32\drivers\etc\hosts -ErrorAction SilentlyContinue; $h=$hosts -match 'autodesk.com'; if($lmgrd -or $clic){Write-Host '  [-] Ket qua: AUTODESK CRACK (Su dung Network License Manager gia lap offline bơi tool X-Force)' -ForegroundColor Red}elif($h){Write-Host '  [-] Ket qua: AUTODESK CRACK (Chan IP nhan dien ban quyen lau cua hang tren file Hosts)' -ForegroundColor Red}elif($svc -and $svc.Status -ne 'Running'){Write-Host '  [-] Ket qua: AUTODESK CRACK (Dich vu cap phep AdskLicensingService bi tat cuong buc de bypass)' -ForegroundColor Red}else{$exe=Get-ChildItem -Path $aDir -Filter 'acad.exe' -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1; if($exe){$sig=Get-AuthenticodeSignature $exe.FullName -ErrorAction SilentlyContinue; if($sig.Status -and $sig.Status -ne 'Valid'){Write-Host '  [-] Ket qua: AUTODESK CRACK (File thuc thi acad.exe bi ghi de mat chu ky so goc)' -ForegroundColor Red}else{Write-Host '  [+] Ket qua: BAN QUYEN XIN 100% (He thong sach, dich vu goc chay on dinh, su dung License hop phap)' -ForegroundColor Green}}else{Write-Host '  [+] Ket qua: BAN QUYEN XIN 100% (Dich vu xac thuc sach, can dang nhap Account de kiem tra quyen thue bao)' -ForegroundColor Green}}}"

echo:
echo %C% KIEM TRA CAC UNG DUNG PDF CHUYEN DUNG:%Res%

:: --- Foxit PDF Editor ---
powershell -Command "$fDir='C:\Program Files (x86)\Foxit Software\Foxit PDF Editor'; if(Test-Path $fDir){$crack=$false; $files=Get-ChildItem -Path $fDir -Include *.exe,*.dll -Recurse; foreach($f in $files){$sig=Get-AuthenticodeSignature $f.FullName -ErrorAction SilentlyContinue; if($sig.Status -and $sig.Status -ne 'Valid' -and $sig.SignerCertificate.Subject -notmatch 'Foxit'){$crack=$true}}; if($crack){Write-Host '  [-] Foxit PDF Editor: PHAT HIEN CRACK (File batch)' -ForegroundColor Red}else{Write-Host '  [+] Foxit PDF Editor: Ban quyen hop le / File he thong sach' -ForegroundColor Green}}"

:: --- Nitro PDF ---
powershell -Command "$nDir=Get-ChildItem -Path 'C:\Program Files\Nitro' -ErrorAction SilentlyContinue; if($nDir){$crack=$false; $files=Get-ChildItem -Path 'C:\Program Files\Nitro' -Include *.exe,*.dll -Recurse -ErrorAction SilentlyContinue; foreach($f in $files){$sig=Get-AuthenticodeSignature $f.FullName -ErrorAction SilentlyContinue; if($sig.Status -and $sig.Status -ne 'Valid' -and $sig.SignerCertificate.Subject -notmatch 'Nitro'){$crack=$true}}; if($crack){Write-Host '  [-] Nitro PDF Pro: PHAT HIEN CRACK (Phat hien file ho tro can thiep vao ma nguon)' -ForegroundColor Red}else{Write-Host '  [+] Nitro PDF Pro: Ung dung hop le / Hoan toan dung file goc' -ForegroundColor Green}}"

:: --- ABBYY FineReader ---
powershell -Command "$abbyyDir = Get-ChildItem -Path 'C:\Program Files\ABBYY FineReader *' -ErrorAction SilentlyContinue; if ($abbyyDir) { $crack = $false; $exeFile = Get-ChildItem -Path $abbyyDir.FullName -Filter 'FineReader.exe' -Recurse -ErrorAction SilentlyContinue; if ($exeFile) { $sigExe = Get-AuthenticodeSignature $exeFile.FullName -ErrorAction SilentlyContinue; if ($sigExe.Status -and $sigExe.Status -ne 'Valid') { $crack = $true } }; $commonLic = 'C:\Program Files\Common Files\ABBYY\FineReader'; if (Test-Path $commonLic) { $licFiles = Get-ChildItem -Path $commonLic -Include *.dll,*.exe -Recurse -ErrorAction SilentlyContinue; foreach ($f in $licFiles) { $sigLic = Get-AuthenticodeSignature $f.FullName -ErrorAction SilentlyContinue; if ($sigLic.Status -and $sigLic.Status -ne 'Valid' -and $sigLic.SignerCertificate.Subject -notmatch 'ABBYY') { $crack = $true } } }; if ($crack) { Write-Host '  [-] ABBYY FineReader: PHAT HIEN CRACK (File thuc thi hoac Module Cap phep bi ghi de mat chu ky so ABBYY!)' -ForegroundColor Red } else { Write-Host '  [+] ABBYY FineReader: Ung dung va Module xac thuc hop le (Dung file goc).' -ForegroundColor Green } }"
