...
powershell -Command "$paths = @('${env:SystemDrive}\Program Files', '${env:SystemDrive}\Program Files (x86)') | Where-Object { Test-Path $_ }; $vbsPaths = Get-ChildItem -Path $paths -Filter 'ospp.vbs' -Recurse -ErrorAction SilentlyContinue | Select-Object -ExpandProperty FullName; foreach ($p in $vbsPaths) { $out = cscript //nologo \"$p\" /dstatus; $keys = $out | Select-String 'Last 5 characters of installed product key: (\w+)'; foreach ($m in $keys) { $k = $m.Matches.Groups.Value; cscript //nologo \"$p\" /unpkey:$k >$null } }; cmdkey /list | Select-String 'MicrosoftOffice|Identity' | ForEach-Object { if ($_ -match 'target=(.*)') { $tg = $matches[1].Trim(); cmdkey /delete:$tg >$null 2>&1 } }; $o365Paths = @(\"${env:LOCALAPPDATA}\Microsoft\Office\16.0\Licensing\", \"${env:APPDATA}\Microsoft\Office\16.0\Licensing\"); foreach ($path in $o365Paths) { if (Test-Path $path) { Remove-Item -Path $path -Recurse -Force >$null 2>&1 } }; $regPaths = @('HKCU:\Software\Microsoft\Office\16.0\Common\Licensing', 'HKCU:\Software\Microsoft\Office\16.0\Common\Identity', 'HKLM:\SOFTWARE\Microsoft\Office\ClickToRun\Configuration', 'HKLM:\SOFTWARE\Microsoft\Office\16.0\Common\Licensing'); foreach ($reg in $regPaths) { if (Test-Path $reg) { Remove-Item -Path $reg -Recurse -Force >$null 2>&1 } }" >nul 2>&1
