...
:BackupnRestore
cls
:: Tu dong tim o dia khac o C de luu du lieu
...
if "%targetDrive%"=="" (
    ...
    set "baseBackupPath=C:\System_Backup"
) else (
    set "baseBackupPath=%targetDrive%:\System_Backup"
)

:: Dinh nghia cac thu muc con
...
echo %C%==========================================%Res%
echo %C%    CONG CU SAO LUU VA PHUC HOI  %Res%
echo %C%==========================================%Res%
...
set /p mainChoice="Nhap lua chon cua ban (0-2): "

if "%mainChoice%"=="1" goto MENUSAOLUU
if "%mainChoice%"=="2" goto MENUPHUCHOI
if "%mainChoice%"=="0" goto Menu
...
set /p bkpChoice="Nhap lua chon sao luu (0-5): "
...
if "%bkpChoice%"=="0" goto BackupnRestore
echo %R%[-] Lua chon khong hop le!%Res%
pause
goto MENUSAOLUU

:BKP_DRIVER
echo. & echo %C%[*] Dang sao luu Driver...%Res%
if not exist "%driverPath%" mkdir "%driverPath%"
powershell -NoProfile -Command "Export-WindowsDriver -Online -Destination '%driverPath%' -ErrorAction SilentlyContinue" >nul
echo %G%[+] Da sao luu Driver xong!%Res% & pause & goto MENUSAOLUU

:BKP_OUTLOOK
echo. & echo %C%[*] Dang sao luu Profile Outlook...%Res%
if not exist "%outlookPath%" mkdir "%outlookPath%"
reg export "HKCU\Software\Microsoft\Office\16.0\Outlook\Profiles" "%outlookPath%\OutlookProfiles.reg" /y >nul 2>&1
echo %G%[+] Da sao luu Profile Outlook xong!%Res% & pause & goto MENUSAOLUU

:BKP_CRED
echo. & echo %C%[*] Dang sao luu Windows Credentials...%Res%
if not exist "%credPath%" mkdir "%credPath%"
cmdkey /list > "%credPath%\vlist.txt"
powershell -NoProfile -Command "$backupFile = '%credPath%\Credentials.bck'; if (Test-Path $backupFile) { Remove-Item $backupFile }; [Windows.Security.Credentials.PasswordVault, Windows.Security.Credentials, ContentType=WindowsRuntime] | Out-Null; $vault = New-Object Windows.Security.Credentials.PasswordVault; $creds = $vault.RetrieveAll(); if ($creds.Count -gt 0) { roundll32.exe keymgr.dll,KRShowKeyMgr } else { Write-Output 'No credentials found' }" >nul 2>&1
echo %G%[+] Da sao luu Credentials xong!%Res% & pause & goto MENUSAOLUU

:BKP_CHROME
echo. & echo %C%[*] Dang sao luu Bookmark Chrome...%Res%
...
if exist "%LocalAppData%\Google\Chrome\User Data\Default\Bookmarks" (
    ...
    echo %G%[+] Da sao luu Bookmark Chrome xong!%Res%
) else (
    echo %R%[-] Khong tim thay file Bookmark Chrome tren may!%Res%
)
pause & goto MENUSAOLUU

:BKP_ALL
echo. & echo %C%[*] Dang sao luu toan bo he thong... Vui long cho...%Res%
if not exist "%driverPath%" mkdir "%driverPath%"
powershell -NoProfile -Command "Export-WindowsDriver -Online -Destination '%driverPath%' -ErrorAction SilentlyContinue" >nul
if not exist "%outlookPath%" mkdir "%outlookPath%"
reg export "HKCU\Software\Microsoft\Office\16.0\Outlook\Profiles" "%outlookPath%\OutlookProfiles.reg" /y >nul 2>&1
...
echo %G%[+] DA SAO LUU TOAN BO CAC MUC THANH CONG!%Res%
pause & goto MENUSAOLUU
...
set /p rstChoice="Nhap lua chon phuc hoi (0-5): "
...
if "%rstChoice%"=="0" goto BackupnRestore
echo %R%[-] Lua chon khong hop le!%Res%
pause
goto MENUPHUCHOI

:RST_DRIVER
echo. & echo %C%[*] Dang phuc hoi Driver...%Res%
if exist "%driverPath%" (
    pnputil.exe /add-driver "%driverPath%\*.inf" /subdirs /install >nul 2>&1
    echo %G%[+] Da phuc hoi Driver xong!%Res%
) else ( echo %R%[-] Khong tim thay thu muc Driver sao luu!%Res% )
pause & goto MENUPHUCHOI

:RST_OUTLOOK
echo. & echo %C%[*] Dang phuc hoi Profile Outlook...%Res%
if exist "%outlookPath%\OutlookProfiles.reg" (
    reg import "%outlookPath%\OutlookProfiles.reg" >nul 2>&1
    echo %G%[+] Da phuc hoi Profile Outlook xong!%Res%
) else ( echo %R%[-] Khong tim thay file dang ky Outlook sao luu!%Res% )
pause & goto MENUPHUCHOI

:RST_CRED
echo. & echo %C%[*] Dang mo trinh nap Credentials...%Res%
if exist "%credPath%" (
    roundll32.exe keymgr.dll,KRShowKeyMgr
    echo %G%[+] Da mo bang dieu khien chung thu!%Res%
) else ( echo %R%[-] Khong tim thay thu muc Credentials sao luu!%Res% )
pause & goto MENUPHUCHOI

:RST_CHROME
echo. & echo %C%[*] Dang phuc hoi Bookmark Chrome...%Res%
if exist "%chromePath%\Bookmarks" (
    ...
    echo %G%[+] Da phuc hoi Bookmark Chrome xong!%Res%
) else ( echo %R%[-] Khong tim thay file Bookmark sao luu!%Res% )
pause & goto MENUPHUCHOI

:RST_ALL
echo. & echo %C%[*] Dang phuc hoi toan bo du lieu... Vui long cho...%Res%
if exist "%driverPath%" pnputil.exe /add-driver "%driverPath%\*.inf" /subdirs /install >nul 2>&1
if exist "%outlookPath%\OutlookProfiles.reg" reg import "%outlookPath%\OutlookProfiles.reg" >nul 2>&1
if exist "%credPath%" roundll32.exe keymgr.dll,KRShowKeyMgr
if exist "%chromePath%\Bookmarks" (
    if not exist "%LocalAppData%\Google\Chrome\User Data\Default" mkdir "%LocalAppData%\Google\Chrome\User Data\Default"
    copy /y "%chromePath%\Bookmarks" "%LocalAppData%\Google\Chrome\User Data\Default\" >nul
)
echo %G%[+] DA PHUC HOI TOAN BO CAC MUC HOAN TAT!%Res%
pause & goto MENUPHUCHOI
