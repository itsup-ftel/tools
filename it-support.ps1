...
echo      %G%5.%Res% Kiem tra Bitlocker   %G%11.%Res% Restart Explorer    %G%17.%Res% Xem Pass Wi-Fi       %G%23.%Res% ----cmn-------
...
powershell -Command "$w = cscript //nologo $env:windir\system32\slmgr.vbs /dli | Out-String; $x = Get-CimInstance -ClassName SoftwareLicensingProduct -Filter \"PartialProductKey is not null and LicenseStatus=1\" -ErrorAction SilentlyContinue | Out-String; if (-not $x -and -not ($w -match 'LICENSE STATUS:\\s*LICENSED')) { Write-Host '  [-] Ket qua: WINDOWS CHUA KICH HOAT HOAC HET HAN DUNG THU' -ForegroundColor Yellow } else { $isKMS = ($w -match 'KMS' -or $x -match 'VOLUME_KMS' -or $w -match '180 days'); $isVolume = ($x -match 'VOLUME_KMS' -or ($x -match 'VOLUME' -and -not ($x -match 'MAK'))); $isOEM = ($w -match 'OEM' -or $x -match 'OEM'); $OEMObj = Get-CimInstance -ClassName Win32_ComputerSystemProduct -ErrorAction SilentlyContinue; $isFakeOEM = ($isOEM -and (-not $OEMObj.DigitalProductId)); if ($isKMS) { Write-Host '  [-] Ket qua: WINDOWS CRACK (Su dung may chu KMS gia lap hoac tool KMSPico/AIO)' -ForegroundColor Red } elif ($isVolume -and $w -match 'VOLUME') { Write-Host '  [-] Ket qua: WINDOWS CRACK (Su dung tool MAS nap Key gia lap ky thuat so HWID luan chuyen kenh Doanh Nghiep)' -ForegroundColor Red } elif ($isFakeOEM) { Write-Host '  [-] Ket qua: WINDOWS CRACK (Gia lap Key OEM, bo mach chu khong co chip ban quyen goc cua nha san xuat)' -ForegroundColor Red } else { Write-Host '  [+] Ket qua: BAN QUYEN XIN (Giay phep Retail hoac OEM tu chip phan cung chuan hang)' -ForegroundColor Green } }"
