...
powershell -Command "$w = cscript //nologo $env:windir\system32\slmgr.vbs /dli; $x = Get-CimInstance -Namespace root\Cimv2\Applications\WindowsLicensing -ClassName SoftwareLicensingProduct -Filter \"ApplicationID='55c92734-d682-4d71-983e-d6ec3f16059f' and PartialProductKey is not null\" -ErrorAction SilentlyContinue; if (-not $x) { Write-Host '  [-] Ket qua: WINDOWS CHUA KICH HOAT (He thong trong troi, khong co Key)' -ForegroundColor Yellow } else { $KMS = $w -match 'KMS' -or $x.Description -match 'VOLUME_KMS'; $Channel = $x.ProductKeyChannel; $Status = $x.LicenseStatus; if ($Status -ne 1) { Write-Host '  [-] Ket qua: WINDOWS CHUA KICH HOAT HOAC HET HAN DUNG THU' -ForegroundColor Yellow } else { if ($KMS) { Write-Host '  [-] Ket qua: WINDOWS CRACK (Su dung may chu KMS gia lap hoac tool AIO)' -ForegroundColor Red } else { if ($Channel -match 'Retail' -or $Channel -match 'OEM') { $OEM = Get-CimInstance -ClassName Win32_ComputerSystemProduct -ErrorAction SilentlyContinue; if ($Channel -match 'OEM' -and (-not $OEM.DigitalProductId)) { Write-Host '  [-] Ket qua: WINDOWS CRACK (Gia lap Key OEM, may khong co chip ban quyen goc)' -ForegroundColor Red } else { Write-Host '  [+] Ket qua: BAN QUYEN XIN 100% (Giay phep Retail / OEM hop phap tu nha san xuat)' -ForegroundColor Green } } else { if ($Channel -match 'Volume') { Write-Host '  [-] Ket qua: WINDOWS CRACK (Su dung tool MAS nap Key gia lap ky thuat so HWID)' -ForegroundColor Red } else { Write-Host '  [-] Ket qua: WINDOWS CRACK (Khong trung khop kenh phan phoi hop le)' -ForegroundColor Red } } } } }"

echo:
echo %C% KIEM TRA BAN QUYEN MICROSOFT OFFICE:%Res%
powershell -Command "$ohook = Test-Path '$env:SystemRoot\system32\spp\tokens\skus\Office*'; $paths = @('${env:ProgramFiles}\Microsoft Office\Office16','${env:ProgramFiles(x86)}\Microsoft Office\Office16'); $status = 'NOT_FOUND'; $channel = 'NONE'; foreach ($p in $paths) { if (Test-Path \"$p\ospp.vbs\") { $o = cscript //nologo \"$p\ospp.vbs\" /dstatus; if ($o -match 'LICENSE STATUS: LICENSED') { $status = 'LICENSED'; if ($o -match 'VOLUME_KMS' -or $o -match 'KMS host') { $channel = 'KMS' } elseif ($o -match 'RETAIL') { $channel = 'RETAIL' } else { $channel = 'VOLUME' } } } }; if ($ohook) { Write-Host '  [-] Ket qua: OFFICE CRACK (Su dung engine OHOOK gia lap chung chi Office 365/2021)' -ForegroundColor Red } else { if ($status -eq 'NOT_FOUND') { Write-Host '  [-] Ket qua: KHONG TIM THAY PHEN BAN OFFICE NAO TREN MAY' -ForegroundColor Yellow } elseif ($status -ne 'LICENSED') { Write-Host '  [-] Ket qua: OFFICE CHUA KICH HOAT HOAC HET HAN BAN QUYEN' -ForegroundColor Yellow } elseif ($channel -eq 'KMS') { Write-Host '  [-] Ket qua: OFFICE CRACK (Su dung may chu KMS gia lap hoac tool AIO Tools)' -ForegroundColor Red } elseif ($channel -eq 'RETAIL') { Write-Host '  [+] Ket qua: BAN QUYEN XIN 100% (Giay phep Retail hop phap hoac Tai khoan Microsoft 365)' -ForegroundColor Green } elseif ($channel -eq 'VOLUME') { Write-Host '  [+] Ket qua: BAN QUYEN DOANH NGHIEP (Su dung khoa Commercial Volume hop le)' -ForegroundColor Green } else { Write-Host '  [-] Ket qua: OFFICE CRACK (Vi pham kenh xac thuc ban quyen)' -ForegroundColor Red } }"
