...
powershell -Command "$w = cscript //nologo $env:windir\system32\slmgr.vbs /dli | Out-String; $x = Get-CimInstance -ClassName SoftwareLicensingProduct -Filter \"PartialProductKey is not null and LicenseStatus=1\" -ErrorAction SilentlyContinue | Out-String; if (-not $x -and -not ($w -match 'LICENSE STATUS:\\s*LICENSED')) { Write-Host '  [-] Ket qua: WINDOWS CHUA KICH HOAT HOAC HET HAN DUNG THU' -ForegroundColor Yellow } else { $isKMS = ($w -match 'KMS' -or $x -match 'VOLUME_KMS' -or $w -match '180 days'); $isVolume = ($x -match 'VOLUME_KMS' -or ($x -match 'VOLUME' -and -not ($x -match 'MAK'))); $isOEM = ($w -match 'OEM' -or $x -match 'OEM'); $OEMObj = Get-CimInstance -ClassName Win32_ComputerSystemProduct -ErrorAction SilentlyContinue; $isFakeOEM = ($isOEM -and (-not $OEMObj.DigitalProductId)); if ($isKMS) { Write-Host '  [-] Ket qua: WINDOWS CRACK (Su dung may chu KMS gia lap hoac tool KMSPico/AIO)' -ForegroundColor Red } elseif ($isVolume -and $w -match 'VOLUME') { Write-Host '  [-] Ket qua: WINDOWS CRACK (Su dung tool MAS nap Key gia lap ky thuat so HWID luan chuyen kenh Doanh Nghiep)' -ForegroundColor Red } elseif ($isFakeOEM) { Write-Host '  [-] Ket qua: WINDOWS CRACK (Gia lap Key OEM, bo mach chu khong co chip ban quyen goc cua nha san xuat)' -ForegroundColor Red } else { Write-Host '  [+] Ket qua: BAN QUYEN XIN (Giay phep Retail hoac OEM tu chip phan cung chuan hang)' -ForegroundColor Green } }"

echo:
echo %C% KIEM TRA BAN QUYEN MICROSOFT OFFICE:%Res%
powershell -Command "$wDir = $null; $regWord = Get-ItemProperty -Path 'HKLM:\\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\Winword.exe' -Name '(default)' -ErrorAction SilentlyContinue; if ($regWord) { $wDir = Split-Path $regWord.'(default)' }; if (-not $wDir) { $pathsDefault = @('${env:ProgramFiles}\Microsoft Office\root\Office16', '${env:ProgramFiles(x86)}\Microsoft Office\root\Office16', '${env:ProgramFiles}\Microsoft Office\Office16', '${env:ProgramFiles(x86)}\Microsoft Office\Office16', '${env:ProgramFiles}\Microsoft Office\Office15', '${env:ProgramFiles(x86)}\Microsoft Office\Office15'); foreach ($p in $pathsDefault) { if (Test-Path \"$p\WINWORD.EXE\") { $wDir = $p; break } } }; if (-not $wDir) { $cmdFind = where /R C:\ WINWORD.EXE 2>$null | Select-Object -First 1; if ($cmdFind) { $wDir = Split-Path $cmdFind } }; if (-not $wDir) { Write-Host '  [-] Ket qua: KHONG TIM THAY PHEN BAN OFFICE NAO DANG CAI DAT TREN MAY' -ForegroundColor Yellow } else { $appName = 'Microsoft Office (' + (Split-Path (Split-Path $wDir) -Leaf) + ' ' + (Split-Path $wDir -Leaf) + ')'; Write-Host \"  [+] Phat hien ung dung: $appName\" -ForegroundColor Cyan; $paths = @(); if (Test-Path \"$wDir\ospp.vbs\") { $paths += $wDir } else { $pParent = Split-Path $wDir; $findVbs = Get-ChildItem -Path $pParent -Filter 'ospp.vbs' -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1; if ($findVbs) { $paths += $findVbs.DirectoryName } }; $ohook = Test-Path '$env:SystemRoot\system32\spp\tokens\skus\Office*'; if ($ohook) { Write-Host '  [-] Ket qua: OFFICE CRACK (Su dung engine OHOOK can thiep gia lap chung chi)' -ForegroundColor Red } else { $crackFile = $false; foreach ($p in $paths) { $exeFiles = Get-ChildItem -Path $p -Filter '*.exe' -ErrorAction SilentlyContinue | Select-Object -First 3; foreach ($f in $exeFiles) { $sig = Get-AuthenticodeSignature $f.FullName -ErrorAction SilentlyContinue; if ($sig.Status -and $sig.Status -ne 'Valid') { $crackFile = $true } else { if ($sig.SignerCertificate) { $isValidRoot = $sig.SignerCertificate.Verify(); if (-not $isValidRoot) { $crackFile = $true } } } } }; if ($crackFile) { Write-Host '  [-] Ket qua: OFFICE CRACK (Phat hien tap tin he thong bi va ma nguon hoac dung chung chi gia)' -ForegroundColor Red } else { if ($paths.Count -gt 0) { $activePath = Join-Path $paths 'ospp.vbs'; $o = cscript //nologo \"$activePath\" /dstatus | Out-String; if ($o -match 'LICENSE STATUS:\\s*LICENSED') { if ($o -match 'VOLUME_KMS' -or $o -match 'KMS host') { Write-Host '  [-] Ket qua: OFFICE CRACK (Su dung may chu KMS gia lap hoac tool AIO Tools)' -ForegroundColor Red } elif ($o -match 'RETAIL') { Write-Host '  [+] Ket qua: BAN QUYEN XIN THUONG MAI (Giay phep Retail nhap Product Key co dinh)' -ForegroundColor Green } else { Write-Host '  [+] Ket qua: BAN QUYEN DOANH NGHIEP (Su dung khoa Commercial Volume hop le)' -ForegroundColor Green } } else { $vScript = Join-Path $paths 'vnextdiag.ps1'; if (-not (Test-Path $vScript)) { $vScript = Join-Path (Split-Path $paths) 'root\Office16\vnextdiag.ps1' }; if (Test-Path $vScript) { $vOut = powershell -ExecutionPolicy Bypass -File \"$vScript\" -List 2>&1 | Out-String; if ($vOut -match 'OlsLicenseState_Licensed') { Write-Host '  [+] Ket qua: BAN QUYEN XIN (Xac thuc thue bao so qua Tai khoan Microsoft 365 Cloud hop le)' -ForegroundColor Green } else { Write-Host '  [-] Ket qua: OFFICE CHUA KICH HOAT HOAC HET HAN BAN QUYEN' -ForegroundColor Yellow } } else { Write-Host '  [-] Ket qua: OFFICE CHUA KICH HOAT HOAC HET HAN BAN QUYEN' -ForegroundColor Yellow } } } else { Write-Host '  [+] Ket qua: BAN QUYEN XIN THUONG MAI (Phien ban Windows Store dac thu kem theo may)' -ForegroundColor Green } } } }"
