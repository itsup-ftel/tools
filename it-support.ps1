...
echo      %G%5.%Res% Kiem tra Bitlocker   %G%11.%Res% Restart Explorer    %G%17.%Res% Xem Pass Wi-Fi       %G%23.%Res% ----licen-------
...
powershell -Command "$w = cscript //nologo $env:windir\system32\slmgr.vbs /dli; $x = Get-CimInstance -Namespace root\Cimv2\Applications\WindowsLicensing -ClassName SoftwareLicensingProduct -Filter \"ApplicationID='55c92734-d682-4d71-983e-d6ec3f16059f' and PartialProductKey is not null\" -ErrorAction SilentlyContinue; if (-not $x) { Write-Host '  [-] Ket qua: WINDOWS CHUA KICH HOAT (He thong trong troi, khong co Key)' -ForegroundColor Yellow } else { $Status = $x.LicenseStatus; if ($Status -ne 1) { Write-Host '  [-] Ket qua: WINDOWS CHUA KICH HOAT HOAC HET HAN DUNG THU' -ForegroundColor Yellow } else { $KMS = $w -match 'KMS' -or $x.Description -match 'VOLUME_KMS'; if ($KMS) { Write-Host '  [-] Ket qua: WINDOWS CRACK (Su dung may chu KMS gia lap hoac tool AIO)' -ForegroundColor Red } else { $Channel = $x.ProductKeyChannel; if ($Channel -match 'Volume') { Write-Host '  [-] Ket qua: WINDOWS CRACK (Su dung tool MAS nap Key gia lap ky thuat so HWID)' -ForegroundColor Red } else { if ($Channel -match 'OEM') { $OEM = Get-CimInstance -ClassName Win32_ComputerSystemProduct -ErrorAction SilentlyContinue; if (-not $OEM.DigitalProductId) { Write-Host '  [-] Ket qua: WINDOWS CRACK (Gia lap Key OEM, bo mach chu khong co chip ban quyen goc)' -ForegroundColor Red } else { Write-Host '  [+] Ket qua: BAN QUYEN XIN 100% (Giay phep OEM hop phap nhung san vao phan cung máy)' -ForegroundColor Green } } else { Write-Host '  [+] Ket qua: BAN QUYEN XIN 100% (Giay phep Retail hop phap mua roi tu nha phat hanh)' -ForegroundColor Green } } } } }"

echo:
echo %C% KIEM TRA BAN QUYEN MICROSOFT OFFICE:%Res%
powershell -Command "$paths = @('${env:ProgramFiles}\Microsoft Office\Office16','${env:ProgramFiles(x86)}\Microsoft Office\Microsoft Office\Office16','${env:ProgramFiles}\Microsoft Office\Office15','${env:ProgramFiles(x86)}\Microsoft Office\Office15'); $activePath = $null; foreach ($p in $paths) { if (Test-Path \"$p\ospp.vbs\") { $activePath = \"$p\ospp.vbs\" } }; if (-not $activePath) { Write-Host '  [-] Ket qua: KHONG TIM THAY PHEN BAN OFFICE NAO TREN MAY' -ForegroundColor Yellow } else { $ohook = Test-Path '$env:SystemRoot\system32\spp\tokens\skus\Office*'; if ($ohook) { Write-Host '  [-] Ket qua: OFFICE CRACK (Su dung engine OHOOK gia lap chung chi Office 365/2021)' -ForegroundColor Red } else { $o = cscript //nologo \"$activePath\" /dstatus; if ($o -notmatch 'LICENSE STATUS: LICENSED') { Write-Host '  [-] Ket qua: OFFICE CHUA KICH HOAT HOAC HET HAN BAN QUYEN' -ForegroundColor Yellow } else { if ($o -match 'VOLUME_KMS' -or $o -match 'KMS host') { Write-Host '  [-] Ket qua: OFFICE CRACK (Su dung may chu KMS gia lap hoac tool AIO Tools)' -ForegroundColor Red } elseif ($o -match 'RETAIL') { Write-Host '  [+] Ket qua: BAN QUYEN XIN 100% (Giay phep Retail hop phap hoac Tai khoan Microsoft 365)' -ForegroundColor Green } else { Write-Host '  [+] Ket qua: BAN QUYEN DOANH NGHIEP (Su dung khoa Commercial Volume hop le)' -ForegroundColor Green } } } }"
...
:: --- Foxit PDF Editor & Foxit PhantomPDF ---
powershell -Command "$fDirs = Get-ChildItem -Path 'C:\Program Files (x86)\Foxit Software\*', 'C:\Program Files\Foxit Software\*' -ErrorAction SilentlyContinue | Where-Object { $_.PSIsContainer -and ($_.Name -match 'Editor' -or $_.Name -match 'PhantomPDF') }; foreach ($d in $fDirs) { $exe = Get-ChildItem -Path $d.FullName -Filter 'Foxit*.exe' -ErrorAction SilentlyContinue | Select-Object -First 1; $dll = Get-ChildItem -Path $d.FullName -Filter 'ComData.dll' -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1; if ($exe) { $crack = $false; foreach ($f in @($exe.FullName, $dll.FullName)) { if ($f -and (Test-Path $f)) { $sig = Get-AuthenticodeSignature $f -ErrorAction SilentlyContinue; if ($sig.Status -and $sig.Status -ne 'Valid' -and $sig.SignerCertificate.Subject -notmatch 'Foxit') { $crack = $true } } }; if ($crack) { Write-Host \"  [-] Ket qua: $($d.Name) BI CRACK (File bi loi chu ky so goc)\" -ForegroundColor Red } else { Write-Host \"  [+] Ket qua: $($d.Name) BAN QUYEN SACH (Dung file goc cua hang)\" -ForegroundColor Green } } }"

:: --- Nitro PDF Pro ---
powershell -Command "$nDirs = Get-ChildItem -Path 'C:\Program Files\Nitro\*', 'C:\Program Files (x86)\Nitro\*' -ErrorAction SilentlyContinue | Where-Object { $_.PSIsContainer -and ($_.Name -match 'Pro') }; foreach ($d in $nDirs) { $exe = \"$($d.FullName)\NitroPDF.exe\"; $dll = \"$($d.FullName)\npcomm.dll\"; if (Test-Path $exe) { $crack = $false; foreach ($f in @($exe, $dll)) { if (Test-Path $f) { $sig = Get-AuthenticodeSignature $f -ErrorAction SilentlyContinue; if ($sig.Status -and $sig.Status -ne 'Valid' -and $sig.SignerCertificate.Subject -notmatch 'Nitro') { $crack = $true } } }; if ($crack) { Write-Host \"  [-] Ket qua: Nitro PDF ($($d.Name)) BI CRACK (Phat hien file can thiep ma nguon)\" -ForegroundColor Red } else { Write-Host \"  [+] Ket qua: Nitro PDF ($($d.Name)) BAN QUYEN SACH (Su dung file goc hop le)\" -ForegroundColor Green } } }"

:: --- ABBYY FineReader ---
powershell -Command "$aDirs = Get-ChildItem -Path 'C:\Program Files\ABBYY FineReader *', 'C:\Program Files (x86)\ABBYY FineReader *' -ErrorAction SilentlyContinue | Where-Object { $_.PSIsContainer }; foreach ($d in $aDirs) { $exe = Get-ChildItem -Path $d.FullName -Filter 'FineReader.exe' -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1; $commonLic = 'C:\Program Files\Common Files\ABBYY\FineReader'; $dll = if (Test-Path $commonLic) { Get-ChildItem -Path $commonLic -Filter '*Lic*.dll' -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1 } else { $null }; if ($exe) { $crack = $false; $aSvc = Get-Service -Name 'ABBYY FineReader * Licensing Service' -ErrorAction SilentlyContinue | Where-Object { $_.DisplayName -match $d.Name.Split(' ')[-1] } | Select-Object -First 1; if ($aSvc -and $aSvc.Status -ne 'Running') { $crack = $true } else { foreach ($f in @($exe.FullName, $dll.FullName)) { if ($f -and (Test-Path $f)) { $sig = Get-AuthenticodeSignature $f -ErrorAction SilentlyContinue; if ($sig.Status -and $sig.Status -ne 'Valid' -and $sig.SignerCertificate.Subject -notmatch 'ABBYY') { $crack = $true } } } }; if ($crack) { Write-Host \"  [-] Ket qua: $($d.Name) BI CRACK (Dich vu cap phep bi tat hoac file bi ghi de ma)\" -ForegroundColor Red } else { Write-Host \"  [+] Ket qua: $($d.Name) BAN QUYEN SACH (He thong giu nguyen module goc)\" -ForegroundColor Green } } }"
