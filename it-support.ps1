...
if /i "%opt%"=="40" goto BackupnRestore
...
powershell -Command "$ak=(Get-ItemProperty 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\SoftwareProtectionPlatform' -ErrorAction SilentlyContinue).BackupProductKeyDefault; if([string]::IsNullOrEmpty($ak) -or $ak -eq 'BBBBB-BBBBB-BBBBB-BBBBB-BBBBB') { Write-Host '1. Windows Active Key: Khong tim thay hoac may dung ban quyen so an key!' -ForegroundColor Red } else { Write-Host '1. Windows Active Key:' $ak -ForegroundColor Green }; $ok=(Get-CimInstance -ClassName SoftwareLicensingService -ErrorAction SilentlyContinue).OA3xOriginalProductKey; if([string]::IsNullOrEmpty($ok)) { Write-Host '2. Windows OEM Key   : Khong co ma Key nao duoc luu trong BIOS/Phan cung!' -ForegroundColor Red } else { Write-Host '2. Windows OEM Key   :' $ok -ForegroundColor Cyan }"
...
if %userChoice%==2 goto clean_windows
if %userChoice%==3 goto clean_office
if %userChoice%==0 goto menu
goto menu
...
powershell -Command "$paths = @($env:SystemDrive + '\Program Files', $env:SystemDrive + '\Program Files (x86)') | Where-Object { Test-Path $_ }; $vbsPaths = Get-ChildItem -Path $paths -Filter 'ospp.vbs' -Recurse -ErrorAction SilentlyContinue | Select-Object -ExpandProperty FullName; foreach ($p in $vbsPaths) { $out = cscript //nologo \"$p\" /dstatus; $keys = $out | Select-String 'Last 5 characters of installed product key: (\w+)'; foreach ($m in $keys) { $k = $m.Matches.Groups[1].Value; cscript //nologo \"$p\" /unpkey:$k >$null } }; $vnext = Get-ChildItem -Path $paths -Filter 'vnextdiag.ps1' -Recurse -ErrorAction SilentlyContinue | Select-Object -ExpandProperty FullName; foreach ($v in $vnext) { $dir = Split-Path $v; cd $dir; $lics = .\vnextdiag.ps1 -action list | Select-String 'LicenseId\s*:\s*([EWW_\w-]+)'; foreach ($l in $lics) { $id = $l.Matches.Groups[1].Value; .\vnextdiag.ps1 -action remove -LicenseId $id >$null 2>&1 } }; cmdkey /list | Select-String 'MicrosoftOffice|Identity|ADAL' | ForEach-Object { if ($_ -match 'Target:\s*(.*)') { $tg = $matches[1].Trim(); cmdkey /delete:$tg >$null 2>&1 } }; $o365Paths = @($env:LOCALAPPDATA + '\Microsoft\Office\16.0\Licensing', $env:APPDATA + '\Microsoft\Office\16.0\Licensing', $env:LOCALAPPDATA + '\Microsoft\Office\Licenses', $env:APPDATA + '\Microsoft\Credentials', $env:LOCALAPPDATA + '\Microsoft\Credentials'); foreach ($path in $o365Paths) { if (Test-Path $path) { Remove-Item -Path $path -Recurse -Force >$null 2>&1 } }; $regPaths = @('HKCU:\Software\Microsoft\Office\16.0\Common\Licensing', 'HKCU:\Software\Microsoft\Office\16.0\Common\Identity', 'HKLM:\SOFTWARE\Microsoft\Office\ClickToRun\Configuration', 'HKLM:\SOFTWARE\Microsoft\Office\16.0\Common\Licensing'); foreach ($reg in $regPaths) { if (Test-Path $reg) { Remove-Item -Path $reg -Recurse -Force >$null 2>&1 } }" >nul 2>&1
...
curl --insecure -u "download:8wjAc41jZ8Fq6Hgd67baCTdoM9uqYcSeRJFU2QvLucL9WreB3h" -L# --ssl-no-revoke -o "%source%\app.zip" "%downloadURL%"
...
echo:     %W%[==^> Tam tat Antivirus de chay kich hoat...]%Res%
powershell -Command "Add-MpPreference -ExclusionPath '%source%'" >nul 2>&1
powershell -Command "Set-MpPreference -DisableRealtimeMonitoring $true -DisableBehaviorMonitoring $true -DisableIOAVProtection $true -DisableIntrusionPreventionSystem $true -DisableScriptScanning $true -SubmitSamplesConsent 2" >nul 2>&1

echo:     %W%[==^> Dang giai nen cong cu kich hoat...]%Res%
...
set "folder64=%ProgramFiles%\%appName%"
set "folder32=%ProgramFiles(x86)%\%appName%"
...
for %%P in ("%foundPath%" "%common64%" "%common32%" "%roamingdesk%" "%localdesk%") do (
    if exist "%%~P" (
        for %%D in (in out) do (
            ...
            for /r "%%~P" %%F in (*.exe) do (
                if exist "%%~F" (
                    netsh advfirewall firewall add rule name="Autodesk_Folder_!appExe!_%%D" dir=%%D program="%%~F" action=block >nul 2>&1
                )
            )
        )
    )
