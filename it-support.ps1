...
powershell -Command "$paths = @(); $regC2R = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Office\ClickToRun\Configuration' -Name 'InstallPath' -ErrorAction SilentlyContinue; if ($regC2R) { $p1 = Join-Path $regC2R.InstallPath 'Office*'; Get-ChildItem -Path $p1 -ErrorAction SilentlyContinue | ForEach-Object { if (Test-Path \"$($_.FullName)\ospp.vbs\") { $paths += $_.FullName } }; $p2 = Join-Path $regC2R.InstallPath 'root\Office*'; Get-ChildItem -Path $p2 -ErrorAction SilentlyContinue | ForEach-Object { if (Test-Path \"$($_.FullName)\ospp.vbs\") { $paths += $_.FullName } } }; $regWord = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\Winword.exe' -Name '(default)' -ErrorAction SilentlyContinue; if ($regWord) { $wDir = Split-Path $regWord.'(default)'; if (Test-Path \"$wDir\ospp.vbs\") { $paths += $wDir } }; $paths = $paths | Select-Object -Unique; if ($paths.Count -eq 0) { Write-Host '  [-] Ket qua: KHONG TIM THAY PHEN BAN OFFICE NAO TREN MAY' -ForegroundColor Yellow } else { $ohook = Test-Path '$env:SystemRoot\system32\spp\tokens\skus\Office*'; if ($ohook) { Write-Host '  [-] Ket qua: OFFICE CRACK (Su dung engine OHOOK can thiep vao tệp tin spp de gia lap chung chi 365/2021)' -ForegroundColor Red } else { $crackFile = $false; foreach ($p in $paths) { $exeFiles = Get-ChildItem -Path $p -Filter '*.exe' -ErrorAction SilentlyContinue | Select-Object -First 3; foreach ($f in $exeFiles) { $sig = Get-AuthenticodeSignature $f.FullName -ErrorAction SilentlyContinue; if ($sig.Status -and $sig.Status -ne 'Valid') { $crackFile = $true } else { if ($sig.SignerCertificate) { $isValidRoot = $sig.SignerCertificate.Verify(); if (-not $isValidRoot) { $crackFile = $true } } } } }; if ($crackFile) { Write-Host '  [-] Ket qua: OFFICE CRACK (Phat hien tap tin he thong bi va ma nguon hoac dung chung chi gia)' -ForegroundColor Red } else { $activePath = \"$($paths[0])\ospp.vbs\"; $o = cscript //nologo \"$activePath\" /dstatus; if ($o -match 'LICENSE STATUS: LICENSED') { if ($o -match 'VOLUME_KMS' -or $o -match 'KMS host') { Write-Host '  [-] Ket qua: OFFICE CRACK (Su dung may chu KMS gia lap hoac tool AIO Tools)' -ForegroundColor Red } elif ($o -match 'RETAIL') { Write-Host '  [+] Ket qua: BAN QUYEN XIN THUONG MAI (Giay phep Retail nhap Product Key co dinh)' -ForegroundColor Green } else { Write-Host '  [+] Ket qua: BAN QUYEN DOANH NGHIEP (Su dung khoa Commercial Volume khoi doanh nghiep hop le)' -ForegroundColor Green } } else { $vScript = Join-Path $paths[0] 'vnextdiag.ps1'; if (Test-Path $vScript) { $vOut = powershell -ExecutionPolicy Bypass -File \"$vScript\" -List 2>&1; if ($vOut -match 'OlsLicenseState_Licensed') { Write-Host '  [+] Ket qua: BAN QUYEN XIN (Xac thuc thue bao so qua Tai khoan Microsoft 365 Cloud hop le)' -ForegroundColor Green } else { Write-Host '  [-] Ket qua: OFFICE CHUA KICH HOAT HOAC HET HAN BAN QUYEN' -ForegroundColor Yellow } } else { Write-Host '  [-] Ket qua: OFFICE CHUA KICH HOAT HOAC HET HAN BAN QUYEN' -ForegroundColor Yellow } } } } }"
