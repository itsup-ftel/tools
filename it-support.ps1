...
echo      %G%5.%Res% Kiem tra Bitlocker   %G%11.%Res% Restart Explorer    %G%17.%Res% Xem Pass Wi-Fi       %G%23.%Res% ----office-------
...
powershell -Command "$w = cscript //nologo $env:windir\system32\slmgr.vbs /dli; $x = Get-CimInstance -Namespace root\Cimv2\Applications\WindowsLicensing -ClassName SoftwareLicensingProduct -Filter \"ApplicationID='55c92734-d682-4d71-983e-d6ec3f16059f' and PartialProductKey is not null\" -ErrorAction SilentlyContinue; if (-not $x) { Write-Host '  [-] Ket qua: WINDOWS CHUA KICH HOAT (He thong trong troi, khong co Key)' -ForegroundColor Yellow } else { $Status = $x.LicenseStatus; $Channel = $x.ProductKeyChannel; $isKMS = ($w -match 'KMS' -or $x.Description -match 'VOLUME_KMS' -or $w -match '180 days'); $isVolume = ($Channel -match 'Volume'); $isOEM = ($Channel -match 'OEM'); $OEMObj = Get-CimInstance -ClassName Win32_ComputerSystemProduct -ErrorAction SilentlyContinue; $isFakeOEM = ($isOEM -and (-not $OEMObj.DigitalProductId)); if ($Status -ne 1) { Write-Host '  [-] Ket qua: WINDOWS CHUA KICH HOAT HOAC HET HAN DUNG THU' -ForegroundColor Yellow } elif ($isKMS) { Write-Host '  [-] Ket qua: WINDOWS CRACK (Su dung may chu KMS gia lap hoac tool KMSPico/AIO)' -ForegroundColor Red } elif ($isVolume) { Write-Host '  [-] Ket qua: WINDOWS CRACK (Su dung tool MAS nap Key gia lap ky thuat so HWID luan chuyen kenh Doanh Nghiep)' -ForegroundColor Red } elif ($isFakeOEM) { Write-Host '  [-] Ket qua: WINDOWS CRACK (Gia lap Key OEM, bo mach chu khong co chip ban quyen goc cua nha san xuat)' -ForegroundColor Red } else { Write-Host '  [+] Ket qua: BAN QUYEN XIN (Giay phep Retail hoac OEM tu chip phan cung chuan hang)' -ForegroundColor Green } }"

echo:
echo %C% KIEM TRA BAN QUYEN MICROSOFT OFFICE:%Res%
powershell -Command "$appName = 'KHONG_TIM_THAY'; $reg = Get-ChildItem -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*', 'HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*' -ErrorAction SilentlyContinue | Get-ItemProperty -ErrorAction SilentlyContinue | Where-Object { $_.DisplayName -match 'Microsoft Office' -and $_.DisplayName -notmatch 'Language' -and $_.DisplayName -notmatch 'Proof' -and $_.DisplayName -notmatch 'OneDrive' -and $_.DisplayName -notmatch 'Tools' }; if ($reg) { $appName = ($reg | Select-Object -First 1).DisplayName } else { $uwp = Get-AppxPackage -Name *Microsoft.Office.Desktop* -ErrorAction SilentlyContinue; if ($uwp) { $appName = 'Microsoft Office (Windows Store App Appx)' } }; if ($appName -eq 'KHONG_TIM_THAY') { Write-Host '  [-] Ket qua: KHONG TIM THAY PHEN BAN OFFICE NAO DANG CAI DAT TREN MAY' -ForegroundColor Yellow } else { Write-Host \"  [+] Phat hien ung dung: $appName\" -ForegroundColor Cyan; $paths = @(); $regC2R = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Office\ClickToRun\Configuration' -Name 'InstallPath' -ErrorAction SilentlyContinue; if ($regC2R) { $p1 = Join-Path $regC2R.InstallPath 'Office*'; Get-ChildItem -Path $p1 -ErrorAction SilentlyContinue | ForEach-Object { if (Test-Path \"$($_.FullName)\ospp.vbs\") { $paths += $_.FullName } }; $p2 = Join-Path $regC2R.InstallPath 'root\Office*'; Get-ChildItem -Path $p2 -ErrorAction SilentlyContinue | ForEach-Object { if (Test-Path \"$($_.FullName)\ospp.vbs\") { $paths += $_.FullName } } }; $regWord = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\Winword.exe' -Name '(default)' -ErrorAction SilentlyContinue; if ($regWord) { $wDir = Split-Path $regWord.'(default)'; if (Test-Path \"$wDir\ospp.vbs\") { $paths += $wDir } }; $paths = $paths | Select-Object -Unique; $ohook = Test-Path '$env:SystemRoot\system32\spp\tokens\skus\Office*'; if ($ohook) { Write-Host '  [-] Ket qua: OFFICE CRACK (Su dung engine OHOOK can thiep gia lap chung chi)' -ForegroundColor Red } else { $crackFile = $false; foreach ($p in $paths) { $exeFiles = Get-ChildItem -Path $p -Filter '*.exe' -ErrorAction SilentlyContinue | Select-Object -First 3; foreach ($f in $exeFiles) { $sig = Get-AuthenticodeSignature $f.FullName -ErrorAction SilentlyContinue; if ($sig.Status -and $sig.Status -ne 'Valid') { $crackFile = $true } else { if ($sig.SignerCertificate) { $isValidRoot = $sig.SignerCertificate.Verify(); if (-not $isValidRoot) { $crackFile = $true } } } } }; if ($crackFile) { Write-Host '  [-] Ket qua: OFFICE CRACK (Phat hien tap tin he thong bi va ma nguon hoac dung chung chi gia)' -ForegroundColor Red } else { if ($paths.Count -gt 0) { $activePath = \"$($paths[0])\ospp.vbs\"; $o = cscript //nologo \"$activePath\" /dstatus; if ($o -match 'LICENSE STATUS: LICENSED') { if ($o -match 'VOLUME_KMS' -or $o -match 'KMS host') { Write-Host '  [-] Ket qua: OFFICE CRACK (Su dung may chu KMS gia lap hoac tool AIO Tools)' -ForegroundColor Red } elif ($o -match 'RETAIL') { Write-Host '  [+] Ket qua: BAN QUYEN XIN THUONG MAI (Giay phep Retail nhap Product Key co dinh)' -ForegroundColor Green } else { Write-Host '  [+] Ket qua: BAN QUYEN DOANH NGHIEP (Su dung khoa Commercial Volume hop le)' -ForegroundColor Green } } else { $vScript = Join-Path $paths[0] 'vnextdiag.ps1'; if (Test-Path $vScript) { $vOut = powershell -ExecutionPolicy Bypass -File \"$vScript\" -List 2>&1; if ($vOut -match 'OlsLicenseState_Licensed') { Write-Host '  [+] Ket qua: BAN QUYEN XIN (Xac thuc thue bao so qua Tai khoan Microsoft 365 Cloud hop le)' -ForegroundColor Green } else { Write-Host '  [-] Ket qua: OFFICE CHUA KICH HOAT HOAC HET HAN BAN QUYEN' -ForegroundColor Yellow } } else { Write-Host '  [-] Ket qua: OFFICE CHUA KICH HOAT HOAC HET HAN BAN QUYEN' -ForegroundColor Yellow } } } else { if ($appName -match 'Store') { Write-Host '  [+] Ket qua: BAN QUYEN XIN THUONG MAI (Phien ban Windows Store dac thu kem theo may)' -ForegroundColor Green } } } } }"
