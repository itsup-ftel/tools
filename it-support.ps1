...
echo      %G%5.%Res% Kiem tra Bitlocker   %G%11.%Res% Restart Explorer    %G%17.%Res% Xem Pass Wi-Fi       %G%23.%Res% ----licapp-------
...
powershell -Command "$aDir='C:\\Program Files\\Autodesk'; if(-not (Test-Path $aDir)){Write-Host '  [-] Ket qua: KHONG CO PHAN MEM AUTODESK/AUTOCAD NAO TREN MAY' -ForegroundColor Gray}else{$lmgrd=Get-Process -Name 'lmgrd','adskflex' -ErrorAction SilentlyContinue; $clic=Test-Path 'C:\\Program Files (x86)\\Common Files\\Autodesk Shared\\AdskLicensing\\adskflex_*.lic'; $hosts=Get-Content $env:windir\system32\drivers\etc\hosts -ErrorAction SilentlyContinue; $isHostsBlock=$hosts -match 'autodesk.com' -or $hosts -match 'genuine-software'; $svc=Get-Service -Name 'AdskLicensingService' -ErrorAction SilentlyContinue; $isSvcStop=($svc -and ($svc.Status -ne 'Running')); $isCrackNLM=($lmgrd -or $clic); $isInvalidSig=$false; $exe=Get-ChildItem -Path $aDir -Filter 'acad.exe' -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1; if($exe){$sig=Get-AuthenticodeSignature $exe.FullName -ErrorAction SilentlyContinue; if($sig.Status -and ($sig.Status -ne 'Valid')){$isInvalidSig=$true}}; if($isCrackNLM){Write-Host '  [-] Ket qua: AUTODESK CRACK (Su dung Network License Manager gia lap May chu offline boi tool X-Force / Magnitude)' -ForegroundColor Red}elif($isHostsBlock){Write-Host '  [-] Ket qua: AUTODESK CRACK (Phat hien chan IP dich vu quet xac thuc Genuine cua Autodesk tren file Hosts)' -ForegroundColor Red}elif($isSvcStop){Write-Host '  [-] Ket qua: AUTODESK CRACK (Dich vu cap phep AdskLicensingService bi tat cuong buc de bypass thong tin)' -ForegroundColor Red}elif($isInvalidSig){Write-Host '  [-] Ket qua: AUTODESK CRACK (File thuc thi acad.exe bi ghi de pha vo chu ky so goc tu hang)' -ForegroundColor Red}else{Write-Host '  [+] Ket qua: BAN QUYEN XIN THUONG MAI (Dich vu goc chay on dinh, su dung dang nhap User License thue bao hop phap)' -ForegroundColor Green}}"

echo:
echo %C% KIEM TRA CAC UNG DUNG PDF CHUYEN DUNG:%Res%

:: --- Foxit PDF Editor & PhantomPDF ---
powershell -Command "$fDirs = Get-ChildItem -Path 'C:\Program Files (x86)\Foxit Software\*', 'C:\Program Files\Foxit Software\*' -ErrorAction SilentlyContinue | Where-Object { $_.PSIsContainer -and ($_.Name -match 'Editor' -or $_.Name -match 'PhantomPDF') }; foreach ($d in $fDirs) { $exe = Get-ChildItem -Path $d.FullName -Filter 'Foxit*.exe' -ErrorAction SilentlyContinue | Select-Object -First 1; if ($exe) { $crack = $false; $files = Get-ChildItem -Path $d.FullName -Filter *.dll -ErrorAction SilentlyContinue | ForEach-Object { $_.FullName }; $files += $exe.FullName; foreach ($f in $files) { $sig = Get-AuthenticodeSignature $f -ErrorAction SilentlyContinue; if ($sig.Status -and $sig.Status -ne 'Valid' -and $sig.SignerCertificate.Subject -notmatch 'Foxit') { $crack = $true } }; if ($crack) { Write-Host \"  [-] Ket qua: $($d.Name) BI CRACK (Phat hien tha tap tin ho tro can thiep he thong hoac file mat chu ky so goc)\" -ForegroundColor Red } else { Write-Host \"  [+] Ket qua: $($d.Name) BAN QUYEN SACH (Su dung dung file nguyen ban tu nha phat hanh)\" -ForegroundColor Green } } }"

:: --- Nitro PDF Pro ---
powershell -Command "$nDirs = Get-ChildItem -Path 'C:\Program Files\Nitro\*', 'C:\Program Files (x86)\Nitro\*' -ErrorAction SilentlyContinue | Where-Object { $_.PSIsContainer -and ($_.Name -match 'Pro') }; foreach ($d in $nDirs) { $exe = Get-ChildItem -Path $d.FullName -Filter 'NitroPDF.exe' -ErrorAction SilentlyContinue | Select-Object -First 1; if ($exe) { $crack = $false; $files = Get-ChildItem -Path $d.FullName -Filter *.dll -ErrorAction SilentlyContinue | ForEach-Object { $_.FullName }; $files += $exe.FullName; foreach ($f in $files) { $sig = Get-AuthenticodeSignature $f -ErrorAction SilentlyContinue; if ($sig.Status -and $sig.Status -ne 'Valid' -and $sig.SignerCertificate.Subject -notmatch 'Nitro') { $crack = $true } }; if ($crack) { Write-Host \"  [-] Ket qua: Nitro PDF ($($d.Name)) BI CRACK (File thuc thi chinh bi ghi de hoac can thiep chuoi nhan dien ma nguon)\" -ForegroundColor Red } else { Write-Host \"  [+] Ket qua: Nitro PDF ($($d.Name)) BAN QUYEN SACH (Ung dung ve dung file goc chu ky xac thuc tron ven)\" -ForegroundColor Green } } }"

:: --- ABBYY FineReader ---
powershell -Command "$aDirs = Get-ChildItem -Path 'C:\Program Files\ABBYY FineReader *', 'C:\Program Files (x86)\ABBYY FineReader *' -ErrorAction SilentlyContinue | Where-Object { $_.PSIsContainer }; foreach ($d in $aDirs) { $exe = Get-ChildItem -Path $d.FullName -Filter 'FineReader.exe' -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1; if ($exe) { $crack = $false; $aSvc = Get-Service -Name 'ABBYY FineReader * Licensing Service' -ErrorAction SilentlyContinue | Where-Object { $_.DisplayName -match $d.Name.Split(' ')[-1] } | Select-Object -First 1; if ($aSvc -and $aSvc.Status -ne 'Running') { $crack = $true } else { $checkList = @($exe.FullName); $commonLic = 'C:\Program Files\Common Files\ABBYY\FineReader'; if (Test-Path $commonLic) { $checkList += (Get-ChildItem -Path $commonLic -Include *.dll,*.exe -Recurse -ErrorAction SilentlyContinue | ForEach-Object { $_.FullName }) }; foreach ($f in $checkList) { $sig = Get-AuthenticodeSignature $f -ErrorAction SilentlyContinue; if ($sig.Status -and $sig.Status -ne 'Valid' -and $sig.SignerCertificate.Subject -notmatch 'ABBYY') { $crack = $true } } }; if ($crack) { Write-Host \"  [-] Ket qua: $($d.Name) BI CRACK (Dich vu kiem tra bi dong bang cuong buc hoac phat hien nho tệp tin .dll mo phong)\" -ForegroundColor Red } else { Write-Host \"  [+] Ket qua: $($d.Name) BAN QUYEN SACH (He thong module ma hoa giu nguyen chung chi bao mat ABBYY)\" -ForegroundColor Green } } }"
