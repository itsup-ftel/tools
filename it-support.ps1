...
powershell -Command "$winword = Get-CimInstance -ClassName CIM_DataFile -Filter \"Name='C:\\\\Program Files\\\\Microsoft Office\\\\root\\\\Office16\\\\WINWORD.EXE' or Name='C:\\\\Program Files (x86)\\Microsoft Office\\\\root\\\\Office16\\\\WINWORD.EXE'\" -ErrorAction SilentlyContinue; if (-not $winword) { $winword = Get-CimInstance -ClassName Win32_Process -Filter \"Name='winword.exe'\" -ErrorAction SilentlyContinue | Select-Object -First 1 }; if (-not $winword) { $winword = Get-CimInstance -ClassName CIM_DataFile -Filter \"Extension='exe' and FileName='winword'\" -ErrorAction SilentlyContinue | Where-Object { $_.Name -match 'Microsoft Office' } | Select-Object -First 1 }; if (-not $winword) { Write-Host '  [-] Ket qua: KHONG TIM THAY PHEN BAN OFFICE NAO DANG CAI DAT TREN MAY' -ForegroundColor Yellow } else { $exePath = if ($winword.Name) { $winword.Name } else { $winword.ExecutablePath }; $wDir = Split-Path $exePath; $appName = 'Microsoft Office (' + (Split-Path (Split-Path $wDir) -Leaf) + ' ' + (Split-Path $wDir -Leaf) + ')'; Write-Host \"  [+] Phat hien ung dung: $appName\" -ForegroundColor Cyan; $paths = @(); if (Test-Path \"$wDir\ospp.vbs\") { $paths += $wDir } else { $pParent = Split-Path $wDir; $findVbs = Get-ChildItem -Path $pParent -Filter 'ospp.vbs' -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1; if ($findVbs) { $paths += $findVbs.DirectoryName } }; $ohook = Test-Path '$env:SystemRoot\system32\spp\tokens\skus\Office*'; if ($ohook) { Write-Host '  [-] Ket qua: OFFICE CRACK (Su dung engine OHOOK can thiep gia lap chung chi)' -ForegroundColor Red } else { $crackFile = $false; foreach ($p in $paths) { $exeFiles = Get-ChildItem -Path $p -Filter '*.exe' -ErrorAction SilentlyContinue | Select-Object -First 3; foreach ($f in $exeFiles) { $sig = Get-AuthenticodeSignature $f.FullName -ErrorAction SilentlyContinue; if ($sig.Status -and $sig.Status -ne 'Valid') { $crackFile = $true } else { if ($sig.SignerCertificate) { $isValidRoot = $sig.SignerCertificate.Verify(); if (-not $isValidRoot) { $crackFile = $true } } } } }; if ($crackFile) { Write-Host '  [-] Ket qua: OFFICE CRACK (Phat hien tap tin he thong bi va ma nguon hoac dung chung chi gia)' -ForegroundColor Red } else { if ($paths.Count -gt 0) { $activePath = Join-Path $paths[0] 'ospp.vbs'; $o = cscript //nologo \"$activePath\" /dstatus; if ($o -match 'LICENSE STATUS: LICENSED') { if ($o -match 'VOLUME_KMS' -or $o -match 'KMS host') { Write-Host '  [-] Ket qua: OFFICE CRACK (Su dung may chu KMS gia lap hoac tool AIO Tools)' -ForegroundColor Red } elif ($o -match 'RETAIL') { Write-Host '  [+] Ket qua: BAN QUYEN XIN THUONG MAI (Giay phep Retail nhap Product Key co dinh)' -ForegroundColor Green } else { Write-Host '  [+] Ket qua: BAN QUYEN DOANH NGHIEP (Su dung khoa Commercial Volume hop le)' -ForegroundColor Green } } else { $vScript = Join-Path $paths[0] 'vnextdiag.ps1'; if (-not (Test-Path $vScript)) { $vScript = Join-Path (Split-Path $paths[0]) 'root\Office16\vnextdiag.ps1' }; if (Test-Path $vScript) { $vOut = powershell -ExecutionPolicy Bypass -File \"$vScript\" -List 2>&1; if ($vOut -match 'OlsLicenseState_Licensed') { Write-Host '  [+] Ket qua: BAN QUYEN XIN (Xac thuc thue bao so qua Tai khoan Microsoft 365 Cloud hop le)' -ForegroundColor Green } else { Write-Host '  [-] Ket qua: OFFICE CHUA KICH HOAT HOAC HET HAN BAN QUYEN' -ForegroundColor Yellow } } else { Write-Host '  [-] Ket qua: OFFICE CHUA KICH HOAT HOAC HET HAN BAN QUYEN' -ForegroundColor Yellow } } } else { Write-Host '  [+] Ket qua: BAN QUYEN XIN THUONG MAI (Phien ban Windows Store dac thu kem theo may)' -ForegroundColor Green } } } }"
